...
impl Plugin for GamePlugin {
    fn build(&self, app: &mut App) {
        app.init_state::<AppState>().add_sub_state::<RoundState>();
...
fn menu_transition(mut next_state: ResMut<NextState<AppState>>) {
...
fn init_transition(mut next_state: ResMut<NextState<RoundState>>) {
...
fn start_of_round_effects_transition(mut next_state: ResMut<NextState<RoundState>>) {
    next_state.set(RoundState::CardSelection);
}

fn card_selection_transition(mut next_state: ResMut<NextState<RoundState>>) {
    next_state.set(RoundState::OrderingInitiative);
}

fn ordering_initiative_transition(mut next_state: ResMut<NextState<RoundState>>) {
    next_state.set(RoundState::CharacterAndMonsterTurns);
}

fn character_and_monster_turns_transition(mut next_state: ResMut<NextState<RoundState>>) {
    next_state.set(RoundState::EndOfRound);
}

fn end_of_round_transition(mut next_state: ResMut<NextState<RoundState>>) {
    next_state.set(RoundState::StartOfRoundEffects);
}
